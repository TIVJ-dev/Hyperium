...
public class NetworkHandler implements INetty, PostConfigHandler, PreSaveHandler {
  ...
  public void handleChat(String s) {
    if (s.toLowerCase(Locale.ENGLISH).contains("reconnecting hyperium connection")) {
      return;
    }
    ...
    for (String s1 : s.split(" ")) {
      ...
      if (s1.contains(".") && !s1.startsWith(".") && !s1.endsWith(".")) {
        ...
        chatStyle.setChatClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL,
            s1.startsWith("http") ? s1 : "http://" + s1));
  ...
  public void handleCrossClientData(UUID uuid, JsonHolder jsonHolder) {
    ...
    if (type.equalsIgnoreCase("twerk_dance")) {
      ...
      if (flipped) {
        ...
        Hyperium.INSTANCE.getHandlers().getFlipHandler()
            .state(uuid, jsonHolder.optInt("flip_state"));
      } else {
